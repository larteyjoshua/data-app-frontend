...
export class GridComponent implements OnInit  {
  ...
  ngOnInit(): void {
    ...
    .subscribe(response => {
      ...
      const containingXs = this.data.filter((x: any) => (x.swift.match(/X/g) || []).length === 4)
      console.log('X contain 4', containingXs)

      const accountLessOrEqual14 = this.data.filter((x: any) => x.account.length <= 14)
      console.log('Account less or equal 14', accountLessOrEqual14)

      const phoneNumberContainX = this.data.filter((x: any) => x.phone_number.includes('x'))
      console.log('Phone number include x', phoneNumberContainX)

      const containEngineer = this.data.filter((x: any) => x.job.includes('Engineer'))
      console.log('Job contain Engineer', containEngineer)

      const phoneNumberBeginWithPlus = this.data.filter((x: any) => x.phone_number.startsWith('+'))
      console.log('Phone begin with +', phoneNumberBeginWithPlus)


      const companyContainLtd = this.data.filter((x: any) => x.company.includes('Ltd'))
    ...
    this.service.getDataWithParameter(100)
    .subscribe(response => {
